<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    <input
        ...
        class="bg-gray-800 text-sm rounded-full w-64 px-4 pl-8 py-1 focus:outline-none focus:shadow-xl mx-1"
        placeholder="Search"
        x-ref="search"
        @keydown.window="
    ...
    @if(strlen($search) >= 2)
        ...
        >
            @if($searchResults->count() > 0)
                <ul>
                    @foreach($searchMovieResults as $result)
                        <li class="border-b border-gray-700">
                            ...
                            >
                                @if($result['poster_path'])
                                    <img
                                        src="https://image.tmdb.org/t/p/w92/{{ $result['poster_path'] }}" class="w-8"
                                        alt="poster">
